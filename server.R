shinyServer(function(input, output, session) {
  ...
  observeEvent(input$map_marker_click, ({
    theEvent <- input$map_marker_click
    ...
    # filterTripsByStop(theEvent$lat, theEvent$lng, 'map')
    ...
    showTripPopup(theEvent$id, theEvent$lat, theEvent$lng, 'map')
  }))
  ...
  tripsInBoundsOrig <- reactive({
    ...
    AggregateAllTrips(tripsOrig, NULL, 'map')
    # Update the heatmap
    RedrawMap('map')
